def climbingLeaderboard(ranked, player):
    ...
    for game in player: 
        print('Game: ' + str(game))
        mid = recursive_search(ranked,game,0,len(ranked)-1)
        score = append_and_score(ranked,game,mid)
        player_scores.append(score)
        print(ranked)
        print(player_scores)
    return player_scores

def recursive_search(ranked,game,low,high): 
    mid = (low + high) // 2
    if high==low: 
        if ranked[high] > game: 
            return min(high + 1,len(ranked))
        elif ranked[high] <= game: 
            return high
    ...
    if ranked[mid] == game: 
        return mid
    elif ranked[mid] > game: 
        mid = recursive_search(ranked,game,mid+1,high) 
    elif ranked[mid] < game: 
        mid = recursive_search(ranked,game,low,mid-1)
    return mid

def append_and_score(ranked,game,ind): 
    ranked.insert(ind,game)
    score = len(set(ranked[:ind])) + 1
    return score
...
if __name__ == '__main__': 
    ranked = [100,90,90,80]
    player = [70,80,105]
    assert climbingLeaderboard(ranked,player) == [4,3,1]
    
    ranked = [100, 90, 90, 80, 75, 60]
    player = [50, 65, 77, 90, 102]
    assert climbingLeaderboard(ranked,player) == [6,5,4,2,1]
    print('Done.')
